...
void right_looking_cholesky_tiled(Tiled_matrix &ft_tiles, int N, std::size_t n_tiles)
{
    for (std::size_t k = 0; k < n_tiles; k++)
    {
        ...
        ft_tiles[k * n_tiles + k] =
            detail::named_dataflow<potrf>("cholesky_tiled", ft_tiles[k * n_tiles + k], N);
        for (std::size_t m = k + 1; m < n_tiles; m++)
        {
            // TRSM:  Solve X * L^T = A
            ft_tiles[m * n_tiles + k] = detail::named_dataflow<trsm>("cholesky_tiled",
                ft_tiles[k * n_tiles + k],
        ...
        {
            // SYRK:  A = A - B * B^T
            ft_tiles[m * n_tiles + m] = detail::named_dataflow<syrk>("cholesky_tiled",
            ...
            {
                // GEMM: C = C - A * B^T
                ft_tiles[m * n_tiles + n] = detail::named_dataflow<gemm>("cholesky_tiled",
                    ft_tiles[m * n_tiles + k],
...
void forward_solve_tiled(Tiled_matrix &ft_tiles, Tiled_vector &ft_rhs, int N, std::size_t n_tiles)
{
    ...
    {
        // TRSM: Solve L * x = a
        ft_rhs[k] = detail::named_dataflow<trsv>("triangular_solve_tiled",
            ft_tiles[k * n_tiles + k],
            ft_rhs[k],
            N,
            Blas_no_trans);
        ...
        {
            // GEMV: b = b - A * a
            ft_rhs[m] = detail::named_dataflow<gemv>("triangular_solve_tiled",
                ft_tiles[m * n_tiles + k],
                ft_rhs[k],
                ft_rhs[m],
                N,
    ...
    Tiled_matrix &ft_tiles, Tiled_matrix &ft_rhs, int N, int M, std::size_t n_tiles, std::size_t m_tiles)
{
    for (std::size_t c = 0; c < m_tiles; c++)
    {
        ...
        {
            // TRSM: solve L * X = A
            ft_rhs[k * m_tiles + c] = detail::named_dataflow<trsm>("triangular_solve_tiled_matrix",
                ft_tiles[k * n_tiles + k],
                ft_rhs[k * m_tiles + c],
                N,
                M,
            ...
            {
                // GEMM: C = C - A * B
                ft_rhs[m * m_tiles + c] = detail::named_dataflow<gemm>("triangular_solve_tiled_matrix",
                    ft_tiles[m * n_tiles + k],
                    ft_rhs[k * m_tiles + c],
                    ft_rhs[m * m_tiles + c],
                    N,
                         ...
                         std::size_t m_tiles)
{
    for (std::size_t k = 0; k < m_tiles; k++)
    {
        ...
        {
            ft_rhs[k] = detail::named_dataflow<gemv>("prediction_tiled",
                ft_tiles[k * n_tiles + m],
                ft_vector[m],
                ft_rhs[k],
                N_row,
    ...
    for (std::size_t i = 0; i < m_tiles; ++i)
    {
        ...
        {  // Compute inner product to obtain diagonal elements of
           // V^T * V  <=> cross(K) * K^-1 * cross(K)^T
            ft_vector[i] = detail::named_dataflow<dot_diag_syrk>("posterior_tiled",
    ...
    for (std::size_t c = 0; c < m_tiles; c++)
    {
        for (std::size_t k = 0; k < m_tiles; k++)
        {
            for (std::size_t m = 0; m < n_tiles; m++)
            {
                ...
                // GEMM:  C = C - A^T * B
                ft_result[c * m_tiles + k] = detail::named_dataflow<gemm>("triangular_solve_tiled_matrix",
...
void vector_difference_tiled(Tiled_vector &ft_minuend, Tiled_vector &ft_subtrahend, int M, std::size_t m_tiles)
{
    ...
    {
        ft_subtrahend[i] =
            detail::named_dataflow<axpy>("uncertainty_tiled", ft_minuend[i], ft_subtrahend[i], M);
    }
}
                        ...
                        std::size_t n_tiles)
{
    ...
    for (std::size_t k = 0; k < n_tiles; k++)
    {
        loss_tiled.push_back(detail::named_dataflow<compute_loss>("loss_tiled",
            ft_tiles[k * n_tiles + k],
            ft_alpha[k],
            ft_y[k],
            N));
    ...
    std::size_t param_idx)
{
    ...
    if (param_idx == 0 || param_idx == 1)  // 0: lengthscale; 1: vertical_lengthscale
    {
        ...
        for (std::size_t i = 0; i < n_tiles; ++i)
        {
            ...
            {
                diag_tiles[i] = detail::named_dataflow<dot_diag_gemm>("trace",
                    ft_invK[i * n_tiles + j],
        ...
        for (std::size_t k = 0; k < n_tiles; k++)
        {
            for (std::size_t m = 0; m < n_tiles; m++)
            {
                inter_alpha[k] = detail::named_dataflow<gemv>("gemv",
        ...
        for (std::size_t j = 0; j < n_tiles; ++j)
        {
            dot = detail::named_dataflow<compute_dot>("grad_right_tiled",
                                inter_alpha[j],
                                ft_alpha[j],
                                dot);
        }
    ...
    else if (param_idx == 2)  // @2: noise_variance
    {
        ...
        for (std::size_t j = 0; j < n_tiles; ++j)
        {
            trace = detail::named_dataflow<compute_trace_diag>("grad_left_tiled",
                                  ft_invK[j * n_tiles + j],
                                  trace,
                                  N);
        }
    ...
    double gradient =
        factor
        * detail::named_dataflow<compute_gradient>("update_hyperparam", trace, dot, N, n_tiles)
              .get();

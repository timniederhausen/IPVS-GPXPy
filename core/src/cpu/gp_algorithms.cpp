    ...
    for (std::size_t i = 0; i < N_row; i++)
    {
        ...
        for (std::size_t j = 0; j < N_col; j++)
        {
            ...
            tile.data()[i * N_col + j] = compute_covariance_function(
                n_regressors, sek_params, row_input.subspan(i_global, n_regressors), col_input.subspan(j_global, n_regressors));
        }
    }
    return tile;
}
...
{
    mutable_tile_data<double> tile(N);
    std::copy(output.begin() + (N * row),
              output.begin() + (N * (row + 1)),
              tile.data());
    return tile;
}

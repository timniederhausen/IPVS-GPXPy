...
TEST_CASE("GP results match known-good values", "[integration]")
{
    ...
    for (std::size_t i = 0, n = results.choleksy.size(); i != n; ++i)
    {
        ...
        {
            INFO("choleksy " << i << " " << j);
            REQUIRE_THAT(results.choleksy[i][j], WithinRel(expected_results.choleksy[i][j], epsilon));
        }
    }
    for (std::size_t i = 0, n = results.losses.size(); i != n; ++i)
    {
        INFO("losses " << i);
        REQUIRE_THAT(results.losses[i], WithinRel(expected_results.losses[i], epsilon));
    }
    for (std::size_t i = 0, n = results.sum.size(); i != n; ++i)
    {
        ...
        {
            INFO("sum " << i << " " << j);
            REQUIRE_THAT(results.sum[i][j], WithinRel(expected_results.sum[i][j], epsilon));
        }
    }
    for (std::size_t i = 0, n = results.full.size(); i != n; ++i)
    {
        ...
        {
            INFO("full " << i << " " << j);
            REQUIRE_THAT(results.full[i][j], WithinRel(expected_results.full[i][j], epsilon));
        }
    }
    for (std::size_t i = 0, n = results.pred.size(); i != n; ++i)
    {
        INFO("pred " << i);
        REQUIRE_THAT(results.pred[i], WithinRel(expected_results.pred[i], epsilon));
    }
}

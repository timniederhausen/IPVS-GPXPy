...
{
    const std::string root("data");  // TODO: get from env?
    const auto results = run_on_data(root + "/training/training_input.txt", root + "/training/training_output.txt", root + "/test/test_input.txt");

    gprat_results expected_results;
    if (!load_or_create_expected_results(root + "/test/output.json", results, expected_results))
    {
